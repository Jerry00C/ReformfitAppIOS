        ...
        ScrollView(.vertical, showsIndicators: false) {
            ScrollViewReader{ value in
                VStack {
                    ...
                    .coordinateSpace(name: "full_vstack")
                }
            }
        }
    }
...
struct CalendarViewforRangePick<DateView>: View where DateView: View {
    @Environment(\.calendar) var calendar
    ...
    var body: some View {
        ScrollViewReader{ value in
            ScrollView(.vertical, showsIndicators: false) {
                ...
                VStack {
                    ForEach(months, id: \.self.id) { month in
                        MonthView(month: month, content: self.content)
                            .id(month.id)
                    }
            ...
            .onAppear(){
                value.scrollTo(months.last.self, anchor: .bottom)
            }
        }
    }
}
...
struct DateRangePicker: View {
    ...
    private var halfYear: DateInterval {
        ...
        let currentDate = Date()
        let startDate = currentDate - 6*30*24*60*60
//        print(DateInterval(start: currentDate, end: endDate).contains(Date()))
        return DateInterval(start: startDate, end: currentDate)
    }
    ...
    var calendarDisplayer: some View{
        CalendarViewforRangePick(interval: halfYear) { date in
